...
import { Plus, Mail, Phone, Calendar, FileText, Globe } from "lucide-react";
...
import { useLeads, useLeadDetail, useCreateLead, useUpdateLead, useAddLeadActivity } from "@/hooks/useCRM";
          ...
          <>
            ...
            <div className="mt-6 space-y-6">
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Stage</span><span className="capitalize">{lead.stage.replace(/_/g, " ")}</span></div>
              </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Pipeline value: <span className="text-primary font-semibold">{fmt(totalPipeline)}</span>
          </p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select name="source" className="w-full mt-1 rounded-md border border-input bg-background px-3 py-2 text-sm">
                    {["Referral", "LinkedIn", "Website", "Conference", "Cold Outreach"].map((s) => (
                      <option key={s} value={s}>{s}</option>
                    ))}
                  </select>
                </div>
                <div><Label>Deal Value</Label><Input name="deal_value" type="number" required className="mt-1" /></div>
                <div>
