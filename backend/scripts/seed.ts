...
async function seed() {
  ...
  const insertedMembers = await db
    .insert(profiles)
    .values([
      { name: "Jordan Reeves", email: "jordan@seekersai.org", avatar: "JR", password: memberPassword, role: "member" as const },
      { name: "Priya Nair",    email: "priya@seekersai.org",  avatar: "PN", password: memberPassword, role: "member" as const },
      { name: "Leo Brandt",    email: "leo@seekersai.org",    avatar: "LB", password: memberPassword, role: "member" as const },
    ])
    .onConflictDoNothing()
    .returning();
